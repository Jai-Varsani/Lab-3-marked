...
function findByUsername(username, callback) {
    const selectClient = (`SELECT * from account where account.username like '${username}';`);
...
function findByNumclient(numOfClient, callback) {
    const selectClient = (`SELECT * from account where num_client like '${numOfClient}';`);
    database.getResult(selectClient, function(err, rows) {
        if (!err) {
            callback(null, rows);
        } else {
...
function findByNumClientInClientTable(numOfClient, callback) {
    const selectClient = (`SELECT * from client where num_client like '${numOfClient}';`);
    database.getResult(selectClient, function(err, rows) {
        if (!err) {
            callback(null, rows);
        } else {
...
function findByUsernameInClientTable(username, callback) {
    const selectClient = (`SELECT * from account where account.username like '${username}';`);
    database.getResult(selectClient, function(err, rows) {
        if (!err) {
            callback(null, rows);
...
function createAccount(numberForClient, username, password, callback) {
    cryptPassword(password, function(err, hash) {
        console.log(`Hash(${password}) -> ${hash}`);
        const insertAccount = (`INSERT INTO account(num_client, username, password) VALUES(${numberForClient}, '${username}', '${hash}');`);
        database.getResult(insertAccount, function(err2, result2) {
            if (!err2) {
                callback(null, result2.affectedRows, numberForClient);
            } else {
                console.log(err2);
                throw err2;
            }
...
module.exports = {
    ...
    createClient,
    findByNumclient,
    findByNumClientInClientTable,
    findByUsernameInClientTable
};